...
class Panel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        es = 0.7
        self.buttons = {
            'extrude': self._gtk.Button("extrude", _("Extrude"), "color4", es),
#            'load': self._gtk.Button("arrow-down", _("Load"), "color3"),
            'unload': self._gtk.Button("arrow-up", _("Unload"), "color2", es),
            'retract': self._gtk.Button("retract", _("Retract"), "color1", es),
            'temperature': self._gtk.Button("heat-up", _("Temperature"), "color4", es),
            'spoolman': self._gtk.Button("spoolman", "Spoolman", "color3", es),
#            'pressure': self._gtk.Button("settings", _("Pressure Advance"), "color2"),
            'retraction': self._gtk.Button("settings", _("Retraction"), "color1", es)
        }
...
#        })
        ...
        for extruder in self._printer.get_tools():
            if self._printer.extrudercount == 1:
                self.labels[extruder] = self._gtk.Button("extruder", "", scale=es)
            else:
                n = self._printer.get_tool_number(extruder)
                self.labels[extruder] = self._gtk.Button(f"extruder-{n}", f"T{n}", scale=es)
...
#            i += 1
        ...
        self.labels['extrude_dist'].set_markup(f"<small>{_('Distance (mm)')}</small>")
        ...
        self.labels['extrude_speed'] = Gtk.Label()
        self.labels['extrude_speed'].set_markup(f"<small>{_('Speed (mm/s)')}</small>")
    ...
    def load_unload(self, widget, direction):
        if direction == "-":           
            if not self.unload_filament:
                ...
                if self._config.get_main_config().getboolean('confirm_unload', True):
                    self._screen._confirm_send_action(
                        widget,
                        _("For soft filament materials such as TPU, please do not use the return material function.") + "\n\n" + _("(This option can be disabled in the settings.)") + "\n\n" + _("Are you sure you want to unload filament?"),
